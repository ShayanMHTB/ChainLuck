...
contract ChainLuckLottery is Ownable, ReentrancyGuard {
    ...
    // Revenue allocation (basis points for precision)
    ...
    function buyTickets(uint256 ticketCount) external nonReentrant {
        ...
        uint256 platformProfit = totalCost - guaranteedWin - grandPrizeAllocation;
        
        // Update pools
        instantPayoutPool += guaranteedWin;
        grandPrizePool += grandPrizeAllocation;
        
        // Process lottery for each ticket
        uint256 totalWinnings = guaranteedWin; // Start with guaranteed win
        
        for (uint256 i = 0; i < ticketCount; i++) {
            uint256 grandPrizeWin = processTicketLottery(msg.sender, i);
            totalWinnings += grandPrizeWin;
        }
        ...
        // Auto-withdraw owner profits if threshold reached
        autoWithdrawProfits();
    }
    ...
    function processTicketLottery(address user, uint256 ticketIndex) internal returns (uint256) {
        // Generate pseudo-random number using multiple sources
    ...
    function claimWins() external nonReentrant {
        ...
        // Ensure we have enough in instant payout pool
        require(instantPayoutPool >= amount, "Insufficient instant payout pool");
